...
export default function InsightDetailModal({ insight, tile, onClose, onViewConsole, insightCharts }: InsightDetailModalProps) {
                    ...
                    <div className="bg-white border-b border-gray-200 px-5 py-2 flex items-center justify-between flex-shrink-0">
                        <div className="flex items-center gap-1.5">
                            {['Period', 'Segment', 'Region'].map((filter) => (
                                <div key={filter} className="flex items-center space-x-1 bg-[#000000] text-white text-[11px] font-medium px-2.5 py-1 rounded-md">
                                    <span>{filter}</span>
                    ...
                    <div className="flex-1 overflow-y-auto p-5">
                        <div className="grid grid-cols-3 gap-5">
                            ...
                            <div className="col-span-2 space-y-4">
                                {/* Card headline — mirrors what was shown on the tile face */}
                                {tile && (
                                    <div className="bg-white rounded-xl border border-gray-200 p-4">
                                        <div className="flex items-start justify-between gap-4">
                                            <div className="min-w-0">
                                                <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest mb-1">{tile.categoryLabel}</p>
                                                <p className="text-3xl font-bold text-gray-900 leading-none">{tile.headlineValue}</p>
                                                <p className="text-sm text-gray-600 mt-1">{tile.headlineLabel}</p>
                                                <div className="flex items-center gap-1.5 mt-2">
                                                    {tile.trendDirection === 'up' ? (
                                                        <TrendingUp className="w-3.5 h-3.5 text-emerald-500 flex-shrink-0" />
                                                    ) : tile.trendDirection === 'down' ? (
                                                        <TrendingDown className="w-3.5 h-3.5 text-red-500 flex-shrink-0" />
                                                    ) : (
                                                        <Minus className="w-3.5 h-3.5 text-gray-400 flex-shrink-0" />
                                                    )}
                                                    <span className={`text-xs font-medium ${tile.trendDirection === 'up' ? 'text-emerald-600' : tile.trendDirection === 'down' ? 'text-red-600' : 'text-gray-500'}`}>
                                                        {tile.trendValue}
                                                    </span>
                                                </div>
                                            </div>
                                            <span className={`flex-shrink-0 text-xs font-semibold px-2.5 py-1 rounded-full ${tile.status === 'good' ? 'bg-emerald-100 text-emerald-700' : tile.status === 'warning' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>
                                                {tile.status === 'good' ? 'On Track' : tile.status === 'warning' ? 'Monitor' : 'Below Target'}
                                            </span>
                                        </div>
                                        <p className="text-xs text-gray-600 mt-3 leading-relaxed border-t border-gray-100 pt-3">{tile.contextLine}</p>
                                    </div>
                                )}
                            ...
                            <div className="col-span-1 flex flex-col space-y-2.5">
                                {recommendations.map((rec: string, idx: number) => {
                                    ...
                                    return (
                                        <div
                                            key={idx}
                                            className={`bg-[#000000] rounded-xl p-3.5 border-l-4 ${colorSet.border}`}
                                        >
                                ...
                                {insight.predictiveInsight && (
                                    <div className="bg-[#000000] rounded-xl p-3.5 border-l-4 border-l-purple-500">
                                        <div className="flex items-start space-x-2.5">
                                ...
                                <button
                                    ...
                                    className={`w-full px-4 py-3 text-sm font-semibold rounded-xl transition-all ${
                                        insight?.consoleAvailable
                                            ? 'text-white bg-[#000000] hover:bg-[#1A1A1A] shadow-lg'
