    ...
    return (
      <select
        ...
        className="px-3 py-1.5 border border-gray-200 rounded-lg text-xs font-medium text-gray-700 bg-white
          focus:outline-none focus:ring-2 focus:ring-[#000000]/20 focus:border-[#000000] transition-all"
      >
        {f.options.map((opt) => (
            ...
            {secondaryFilters.length > 0 && (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                  showSecondary ? 'bg-[#F0F0F0] text-[#000000]' : 'text-gray-500 hover:bg-gray-100'
                }`}
              >
          ...
          <div className="flex items-center gap-2 pb-2">
            ...
            {activeChips.map((f) => {
              ...
              return (
                <span
                  key={f.id}
                  className="inline-flex items-center gap-1 px-2 py-0.5 bg-[#F0F0F0]/50 text-[#000000] rounded-full text-xs font-medium"
                >
