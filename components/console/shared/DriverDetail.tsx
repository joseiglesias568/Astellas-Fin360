  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-lg font-bold text-[#000000]">{driver.name}</h2>
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="bg-gray-50 rounded-lg p-3">
          ...
          <div className="flex items-baseline gap-1 mt-1">
            <span className="text-xl font-bold text-[#000000]">{driver.value}</span>
        ...
        {driver.target && (
          <div className="bg-gray-50 rounded-lg p-3">
            ...
            <p className="text-xl font-bold text-[#000000] mt-1">{driver.target}</p>
          </div>
        )}
        {driver.gap && (
          <div className="bg-gray-50 rounded-lg p-3">
      ...
      {driver.trendData.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <h3 className="text-sm font-semibold text-[#000000] mb-3">Trend Analysis</h3>
      ...
      {driver.subDrivers.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <h3 className="text-sm font-semibold text-[#000000] mb-3">Driver Contributions</h3>
                ...
                <div key={sd.name} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-5 bg-gray-100 rounded-full overflow-hidden relative">
                    <motion.div
                      ...
                      className={`h-full rounded-full ${isPositive ? 'bg-[#000000]' : 'bg-red-400'}`}
                    />
                  </div>
      ...
      {driver.variance && (
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-[#000000] rounded-lg p-3 text-white">
          ...
          <div className="bg-gray-50 rounded-lg p-3">
            ...
            <p className="text-lg font-bold text-[#000000] mt-1">{driver.variance.plan}</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-3">
            <p className="text-xs text-gray-500">vs Prior Year</p>
            <p className="text-lg font-bold text-[#000000] mt-1">{driver.variance.priorYear}</p>
          </div>
        </div>
      )}
      ...
      {/* AI Insight */}
      {driver.aiInsight && (
        <div className="p-4 bg-[#F0F0F0]/40 rounded-xl border border-[#000000]/10">
          <div className="flex items-center gap-1.5 mb-2">
            <Sparkles className="w-4 h-4 text-[#000000]" />
            <span className="text-xs font-semibold text-[#000000]">AI Analysis</span>
          </div>
          <p className="text-sm text-[#000000] leading-relaxed">{driver.aiInsight}</p>
        </div>
      )}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {driver.crossRefs.map((ref) => (
              <span
                key={ref.consoleId}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-xs font-medium text-gray-700 hover:border-[#000000]/30 hover:text-[#000000] cursor-pointer transition-colors"
              >
