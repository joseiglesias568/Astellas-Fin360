    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
      <div className="p-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h2 className="text-lg font-bold text-[#000000]">{brief.title}</h2>
...
function TimelineView({ events }: { events: TimelineEvent[] }) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
      <h2 className="text-lg font-bold text-[#000000] mb-6">Key Developments</h2>
            ...
            return (
              ...
              >
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <h3 className="text-sm font-semibold text-[#000000]">{event.title}</h3>
...
function CompetitiveView({ data }: { data: CompetitiveData }) {
  return (
    <div className="space-y-4">
      ...
      {/* Competitor Actions */}
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
        <h3 className="text-sm font-semibold text-[#000000] mb-4">Recent Competitor Activity</h3>
        <div className="space-y-3">
          {data.competitors.map((c, i) => (
            <div key={i} className="bg-gray-50 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-sm font-semibold text-[#000000]">{c.competitor}</span>
      ...
      {themes.map((theme, idx) => (
        ...
        >
          <div className="flex items-start justify-between mb-3">
            <div>
              <h3 className="text-sm font-semibold text-[#000000]">{theme.name}</h3>
              <p className="text-xs text-gray-500 mt-0.5">{theme.description}</p>
            </div>
            <div className="flex items-center gap-3">
              <StatusBadge status={theme.status} />
              <span className="text-sm font-bold text-[#000000]">{theme.progress}%</span>
            </div>
          </div>
              ...
              <ul className="space-y-1.5">
                {theme.initiatives.map((init, i) => (
                  <li key={i} className="text-xs text-gray-700 flex items-start gap-2">
                    <CheckCircle2 className="w-3.5 h-3.5 text-[#000000] mt-0.5 flex-shrink-0" />
                    {init}
                  </li>
                ))}
              </ul>
                ...
                {theme.kpis.map((kpi, i) => (
                  <div key={i} className="flex items-center justify-between text-xs">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-[#000000]">{kpi.current}</span>
