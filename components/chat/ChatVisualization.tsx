...
const COLORS = ['#000000', '#1A1A1A', '#10B981', '#3B82F6', '#F59E0B', '#EF4444', '#8B5CF6'];
  ...
  useEffect(() => {
    if (spec.type === 'mermaid' && spec.mermaidCode) {
      import('mermaid').then((m) => {
        m.default.initialize({ startOnLoad: false, theme: 'neutral', themeVariables: { primaryColor: '#F0F0F0', primaryTextColor: '#000000', primaryBorderColor: '#000000', lineColor: '#000000' } });
  ...
  if (spec.type === 'mermaid') {
    return (
      <div className="my-3 p-3 bg-white rounded-lg border border-gray-200 shadow-sm">
        <p className="text-xs font-semibold text-[#000000] mb-2">{spec.title}</p>
        {mermaidSvg ? (
    ...
    <div className="my-3 p-3 bg-white rounded-lg border border-gray-200 shadow-sm">
      ...
      <ResponsiveContainer width="100%" height={200}>
        {spec.type === 'bar' ? (
          ...
          <BarChart data={data}><Bar dataKey="value" fill="#000000" /></BarChart>
        )}
      </ResponsiveContainer>
    </div>
  );
