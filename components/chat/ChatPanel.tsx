      ...
      if (part.type === 'tool-invocation') {
        ...
        return (
          <span key={i} className="flex items-center gap-1.5 text-xs text-gray-500 italic my-1">
            <span className="w-3 h-3 border-2 border-[#000000] border-t-transparent rounded-full animate-spin inline-block" />
            Querying {toolName}...
          </span>
        );
      }
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 bg-[#000000] text-white">
                ...
                <div className="flex justify-start">
                  <div className="bg-gray-100 rounded-xl px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      <div className="w-2 h-2 bg-[#000000] rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                      <div className="w-2 h-2 bg-[#000000] rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                      <div className="w-2 h-2 bg-[#000000] rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="border-t border-gray-200 p-3">
              <form onSubmit={handleSubmit} className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#000000]/30 focus:border-[#000000]/30"
                  disabled={isLoading}
                />
                <button
                  type="submit"
                  disabled={isLoading || !inputValue.trim()}
                  className="p-2.5 bg-[#000000] hover:bg-[#1A1A1A] text-white rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
