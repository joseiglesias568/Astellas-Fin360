    ...
    <div className={`relative ${className}`}>
      ...
      <motion.button
        ...
        className={`w-full text-left bg-white rounded-xl border border-gray-200 p-4 shadow-sm transition-all ${
          detail ? 'cursor-pointer hover:shadow-md hover:border-[#000000]/30' : ''
        } ${expanded ? 'ring-2 ring-[#000000]/20' : ''}`}
      ...
      >
        ...
        {detail && !expanded && (
          <div className="flex items-center gap-1 mt-2 text-xs text-[#000000]">
        ...
        {expanded && detail && (
          ...
          >
            <div className="mt-2 bg-white rounded-xl border border-gray-200 shadow-md p-5">
                        ...
                        <div key={d.label} className="flex items-center gap-2">
                          ...
                          <div className="flex-1 h-4 bg-gray-100 rounded-full overflow-hidden">
                            <motion.div
                              initial={{ width: 0 }}
                              animate={{ width: `${pct}%` }}
                              className={`h-full rounded-full ${d.value >= 0 ? 'bg-[#000000]' : 'bg-red-400'}`}
                            />
                          </div>
              ...
              {/* AI Insight */}
              {detail.aiInsight && (
                <div className="p-3 bg-[#F0F0F0]/40 rounded-lg border border-[#000000]/10 mb-4">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <p className="text-xs font-semibold text-[#000000] mb-1">AI Insight</p>
                      <p className="text-sm text-[#000000]">{detail.aiInsight}</p>
                    </div>
              ...
              {reportLink && (
                <Link
                  href={reportLink}
                  className="inline-flex items-center gap-1.5 text-sm font-medium text-[#000000] hover:text-[#000000] transition-colors"
                >
