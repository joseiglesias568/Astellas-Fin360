...
const cadenceColors: Record<string, { bg: string; text: string }> = {
  Monthly: { bg: 'bg-[#000000]/10', text: 'text-[#000000]' },
  ...
  const cardContent = (
    <motion.div
      ...
      className={`bg-white rounded-xl border border-gray-200 p-6 h-full flex flex-col transition-all duration-200 ${
        isReady
          ? 'group hover:shadow-lg hover:border-[#000000]/30 cursor-pointer'
          : 'opacity-60 cursor-not-allowed'
      }`}
    >
      {/* Top row: icon + cadence badge */}
      <div className="flex items-start justify-between mb-4">
        <div className={`p-2.5 rounded-lg ${isReady ? 'bg-[#000000]/8 group-hover:bg-[#000000]/12' : 'bg-gray-100'} transition-colors`}>
          <Icon className={`w-5 h-5 ${isReady ? 'text-[#000000]' : 'text-gray-400'}`} />
        </div>
      ...
      {/* Meeting name */}
      <h3 className={`text-[15px] font-semibold mb-1.5 transition-colors ${isReady ? 'text-gray-900 group-hover:text-[#000000]' : 'text-gray-600'}`}>
        {meeting.name}
      </h3>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              <div className="p-2.5 bg-[#000000]/10 rounded-xl">
                <Presentation className="w-6 h-6 text-[#000000]" />
              </div>
              <div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-8">
            ...
            <motion.div
              ...
              className="group bg-white rounded-xl border-2 border-dashed border-gray-200 p-6 hover:border-[#000000]/40 hover:bg-[#000000]/[0.02] transition-all duration-200 cursor-pointer h-full flex flex-col items-center justify-center text-center min-h-[220px]"
            >
              <div className="p-3 rounded-full bg-gray-50 group-hover:bg-[#000000]/10 transition-colors mb-3">
                <Plus className="w-6 h-6 text-gray-400 group-hover:text-[#000000] transition-colors" />
              </div>
              <h3 className="text-sm font-semibold text-gray-600 group-hover:text-[#000000] transition-colors mb-1">
