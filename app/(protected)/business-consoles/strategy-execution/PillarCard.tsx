...
const statusConfig = {
  'on-track': { color: '#000000', bg: 'bg-[#F0F0F0]', text: 'text-[#000000]', label: 'On Track', icon: CheckCircle2, ring: 'stroke-[#000000]' },
  'ahead': { color: '#000000', bg: 'bg-[#F0F0F0]', text: 'text-[#000000]', label: 'Ahead', icon: TrendingUp, ring: 'stroke-[#000000]' },
...
const kpiStatusColors: Record<string, { bg: string; text: string; border: string }> = {
  'on-track': { bg: 'bg-[#F0F0F0]', text: 'text-[#000000]', border: 'border-[#000000]/20' },
  'ahead': { bg: 'bg-[#F0F0F0]', text: 'text-[#000000]', border: 'border-[#000000]/20' },
...
export default function PillarCard({ pillar, isExpanded, onToggle }: PillarCardProps) {
  ...
  return (
    <motion.div
      layout
      className={`bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden transition-shadow ${isExpanded ? 'shadow-md ring-1 ring-[#000000]/10' : 'hover:shadow-md'}`}
    >
      {/* Card Header */}
      <button
        onClick={onToggle}
        className="w-full text-left p-5 focus:outline-none focus:ring-2 focus:ring-[#000000]/20 rounded-xl"
      >
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <h3 className="text-sm font-bold text-[#000000] truncate">{pillar.name}</h3>
                    ...
                    <div className="relative h-2 bg-gray-100 rounded-full overflow-hidden">
                      <motion.div
                        ...
                        className={`h-full rounded-full ${
                          kpi.status === 'on-track' || kpi.status === 'ahead' ? 'bg-[#000000]' :
