...
const RACI: RaciRow[] = [
  ...
  { activity: 'KPI definition & validation (Core OP/Revenue)',  cfo: 'C', fpaSponsor: 'A', dataOwner: 'R', it: 'I', accenture: 'C' },
...
const SUCCESS_METRICS: SuccessMetric[] = [
  ...
  { category: 'Time to Insight',  metric: 'Core OP variance explained (hours to root cause)',      baseline: '4–8 hrs',   poc: '< 2 hr', pilot: '< 30 min', production: 'Real-time' },
  ...
  { category: 'Business Impact',  metric: 'Revenue forecast accuracy (MAPE)',                    baseline: '—',         poc: '—',      pilot: '< 5%',     production: '< 3%'      },
...
const ROI_INPUTS: RoiInput[] = [
  ...
  { id: 'core_op_margin', label: 'Core OP Margin Improvement', unit: '0.1pp increment', defaultValue: 2, step: 1, min: 0, max: 10, impactPerUnit: 2.1, description: 'Each 0.1pp improvement on ¥2,139B revenue base ≈ +¥2.1B Core OP' },
  { id: 'smt_acceleration', label: 'SMT Savings Acceleration', unit: '¥5B increment', defaultValue: 1, step: 1, min: 0, max: 8, impactPerUnit: 5, description: 'Each ¥5B additional SMT savings ahead of ¥40B FY26 target' },
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="bg-white rounded-xl border border-gray-200 p-5">
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="text-right">
            ...
            <p className="text-2xl font-black text-gray-900">¥{totalRoi.toFixed(1)}B+</p>
          ...
          {ROI_INPUTS.map(r => (
            <div key={r.id} className="p-3 rounded-lg bg-gray-50 border border-gray-100">
              ...
              <div className="flex items-center justify-between mt-1.5">
                ...
                <p className="text-[10px] font-bold text-gray-700 flex-shrink-0 ml-2">+¥{(values[r.id] * r.impactPerUnit).toFixed(1)}B</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-gray-900 rounded-xl p-6">
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-gray-400 text-xs max-w-lg">All four foundational steps are addressed. Book your Demo Workshop to kick off the journey from Day 1 to Production — with Astellas's Finance360 as your intelligent system of intelligence for financial decision-making.</p>
          </div>
