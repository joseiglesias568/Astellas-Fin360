...
const USE_CASES_CATALOG = [
  // POC-ready
  { id: 'iet-revenue',   label: 'XTANDI Franchise & Revenue Dashboard',        phase: 'poc',  domain: 'XTANDI Franchise',  weeks: 0.5, icon: TrendingUp },
  { id: 'orders-backlog',label: 'PADCEV Launch & Pipeline Monitor',           phase: 'poc',  domain: 'XTANDI Franchise',  weeks: 0.5, icon: BarChart3  },
  { id: 'ebitda-bridge', label: 'Core EPS Bridge & Margin Decomposition',    phase: 'poc',  domain: 'XTANDI Franchise',  weeks: 0.5, icon: DollarSign },
  { id: 'ofse-revenue',  label: 'Geographic Revenue Performance',            phase: 'poc',  domain: 'Strategic Brands',  weeks: 0.5, icon: TrendingUp },
  { id: 'cost-bridge',   label: 'Core OP Cost Bridge & Productivity Tracking',phase: 'poc',  domain: 'Cost & Ops',       weeks: 0.5, icon: Shield     },
  { id: 'scenario-oil',  label: 'Scenario Modeling: XTANDI IRA & FX Cycles', phase: 'poc',  domain: 'Planning',         weeks: 0.5, icon: Shield     },
  { id: 'nlq-qa',        label: 'Natural Language Financial Q&A',            phase: 'poc',  domain: 'AI',               weeks: 0.5, icon: Cpu        },
  // Pilot-ready
  { id: 'iet-geo',       label: 'US/Japan Revenue by Product & Indication',  phase: 'pilot',domain: 'XTANDI Franchise',  weeks: 1.0, icon: Globe      },
  { id: 'ofse-rig',      label: 'EM/China Revenue & FX Correlation',         phase: 'pilot',domain: 'Strategic Brands',  weeks: 1.0, icon: BarChart3  },
  { id: 'supply-chain',  label: 'Supply Chain & Procurement Intelligence',   phase: 'pilot',domain: 'Cost & Ops',    weeks: 1.5, icon: Globe      },
  { id: 'ebitda-fcst',   label: '18-Month Rolling Core OP Forecast',         phase: 'pilot',domain: 'Planning',      weeks: 1.5, icon: BarChart3  },
  ...
  { id: 'aftermarket',   label: 'VEOZAH / IZERVAY Launch Optimization',      phase:'production',domain: 'Strategic Brands', weeks: 2.0, icon: TrendingUp },
...
const DATA_SOURCES_CATALOG = [
  ...
  { id: 'market',      label: 'External Market & Pharma Data',         pocWeeks: 0.5, pilotWeeks: 1.0, category: 'External',     complexity: 'Low'    },
];
...
const PHASE_META = {
  ...
  production: { label: 'Production',       short: 'Production',  color: '#000000', bg: 'bg-gray-900', border: 'border-gray-800',  icon: TrendingUp },
} as const;
...
const GATE_CRITERIA: Record<Phase, string[]> = {
  poc: [
    ...
    'POC data model documented and signed off by Data Steward (Core OP, Revenue, XTANDI TRx, Core EPS definitions reconciled to source)',
...
const RISKS: Record<Phase, { risk: string; mitigation: string; severity: 'High'|'Med'|'Low' }[]> = {
  poc: [
    ...
    { risk: 'Astellas Core OP and product revenue KPI definitions differ between Finance360 model and source system of record', mitigation: 'Mandatory data dictionary review in Week 1; document reconciliation log for all segment-level KPIs', severity: 'High' },
    ...
    { risk: 'AI outputs not grounded in Astellas pharma business context', mitigation: 'Allocate dedicated AI calibration sprint in Week 2; validate against 10+ known historical narratives', severity: 'Med' },
  ],
  pilot: [
    ...
    { risk: 'Data quality issues in SAP cause Core OP bridge calculation discrepancies', mitigation: 'Implement data quality monitoring dashboard; establish data issue triage SLA with source owners', severity: 'High' },
  ...
  production: [
    { risk: 'Enterprise-wide change management under-resourced across Astellas five-segment structure (US/EM/Japan/Intl/China)', mitigation: 'Dedicated change lead from project inception; phased rollout by segment and region with champions network', severity: 'High' },
    { risk: 'Data governance gaps at scale lead to Core OP/revenue certification disputes between segments', mitigation: 'Establish Finance360 Data Governance Council with quarterly certification reviews before Production launch', severity: 'High' },
              ...
              <div key={phase} className="grid grid-cols-[100px_1fr_60px] items-center gap-3">
                ...
                <div className="h-5 bg-gray-100 rounded-full overflow-hidden">
                  <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.6, delay: i * 0.1 }}
                    className="h-full rounded-full" style={{ backgroundColor: meta.color === '#000000' ? '#1F2937' : meta.color }} />
                </div>
              ...
              <button key={uc.id} onClick={() => toggleUC(uc.id)}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {isNewInThisPhase && <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded" style={{ backgroundColor: `${meta.color}20`, color: meta.color === '#000000' ? '#374151' : meta.color }}>New in {meta.short}</span>}
                  </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div className="space-y-1.5">
            ...
            {(phase === 'poc' ? ['CFO / Finance Leadership', 'FP&A Team', 'Data Steward / Controller']
              ...
              : ['Finance Leadership', 'FP&A / Analytics', 'Segment Finance (US/EM/Japan/Intl/China)', 'Investor Relations', 'Exec / Board', 'IT / Data Teams']
            ).map((a, i) => (
...
export default function ScopingWorksheetClient() {
  ...
  const [pilotState, setPilotState] = useState<PhaseState>({ useCases: new Set(['iet-revenue','orders-backlog','ebitda-bridge','ofse-revenue','cost-bridge','scenario-oil','ebitda-fcst','fx-hedge','mor-auto','ai-commentary']), dataSources: new Set(['erp','orders','crm','planning','treasury']), userCount: 25 });
