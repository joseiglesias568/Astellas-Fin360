...
const markdownComponents = {
  ...
  h1: ({ children }: any) => <h1 className="text-lg font-bold mb-3 mt-4 first:mt-0 text-[#000000]">{children}</h1>,
  h2: ({ children }: any) => <h2 className="text-base font-semibold mb-2 mt-3 first:mt-0 text-[#000000]">{children}</h2>,
  h3: ({ children }: any) => <h3 className="text-sm font-semibold mb-2 mt-3 first:mt-0 text-[#000000]">{children}</h3>,
  ...
  strong: ({ children }: any) => <strong className="font-semibold text-[#000000]">{children}</strong>,
  ...
  code: ({ children, className }: any) => {
    ...
    return <code className="bg-[#F0F0F0]/50 text-[#000000] px-1.5 py-0.5 rounded text-xs font-mono">{children}</code>;
  },
  pre: ({ children }: any) => <pre className="my-3">{children}</pre>,
  blockquote: ({ children }: any) => (
    <blockquote className="border-l-[3px] border-[#000000]/30 pl-4 italic text-gray-600 my-3">{children}</blockquote>
  ),
  table: ({ children }: any) => (
  ...
  th: ({ children }: any) => <th className="px-3 py-2 text-left font-semibold text-[#000000] border-b border-gray-200">{children}</th>,
...
function renderMessageParts(parts: any[] | undefined) {
  ...
  return parts.map((part: any, i: number) => {
    ...
    if (part.type === 'tool-invocation') {
      ...
      return (
        <motion.span
          ...
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="flex items-center gap-2 text-xs text-[#000000]/70 my-2"
        >
          <span className="w-3.5 h-3.5 border-2 border-[#000000] border-t-transparent rounded-full animate-spin inline-block" />
    ...
    if (typeof part.type === 'string' && part.type.startsWith('tool-')) {
      ...
      return (
        <span key={i} className="flex items-center gap-2 text-xs text-[#000000]/70 my-2">
          <span className="w-3.5 h-3.5 border-2 border-[#000000] border-t-transparent rounded-full animate-spin inline-block" />
          <span className="font-medium">Querying {toolName}...</span>
        </span>
      );
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="p-1.5 rounded-lg text-gray-400 hover:text-[#000000] hover:bg-[#F0F0F0]/50 transition-all"
      title="Copy to clipboard"
    >
      {copied ? <Check className="w-3.5 h-3.5 text-[#000000]" /> : <Clipboard className="w-3.5 h-3.5" />}
    </button>
  );
}
            ...
            <div className="max-w-2xl mx-auto px-6 pt-20 pb-8">
              ...
              >
                ...
                <div className="relative w-20 h-20 mx-auto mb-8">
                  <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-[#000000] to-[#000000] shadow-xl shadow-[#000000]/20" />
                  <div className="absolute inset-0 rounded-3xl flex items-center justify-center">
                    <Sparkles className="w-9 h-9 text-white" />
                  </div>
                  <div className="absolute -inset-1 rounded-3xl bg-gradient-to-br from-[#000000]/20 to-transparent blur-xl -z-10" />
                </div>
                <h1 className="text-3xl font-bold text-[#000000] mb-3 tracking-tight">
              ...
              >
                {SUGGESTED_PROMPTS.map(({ icon: Icon, text, subtitle, gradient, iconBg }, idx) => (
                  <motion.button
                    ...
                    className={`group relative flex items-start gap-3.5 p-4 bg-gradient-to-br ${gradient} border border-gray-200/60 rounded-2xl hover:border-[#000000]/20 hover:shadow-lg hover:shadow-[#000000]/5 transition-all duration-300 text-left`}
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <span className="text-sm text-gray-700 group-hover:text-[#000000] leading-snug block">{text}</span>
                    </div>
                  </motion.button>
                ))}
              </motion.div>
                ...
                >
                  {msg.role === 'user' ? (
                    <div className="max-w-[75%]">
                      <div className="bg-[#000000] text-white rounded-2xl rounded-br-md px-5 py-3 text-sm shadow-sm">
                    ...
                    <div className="max-w-[85%] flex gap-3">
                      <div className="flex-shrink-0 mt-1">
                        <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-[#000000] to-[#000000] flex items-center justify-center shadow-sm">
                          <Sparkles className="w-4 h-4 text-white" />
                        </div>
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1.5">
                          <span className="text-xs font-semibold text-[#000000]">Astellas AI</span>
                    ...
                    <div className="max-w-[85%] flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1.5">
                          <span className="text-xs font-semibold text-[#000000]">Astellas AI</span>
                          {status === 'streaming' && (
                            <span className="text-[10px] text-[#000000] font-medium flex items-center gap-1">
                              <span className="w-1.5 h-1.5 bg-[#000000] rounded-full animate-pulse" />
                              Responding
                            </span>
                          )}
                        </div>
                      ...
                      <div className="bg-white rounded-2xl rounded-tl-md px-5 py-4 border border-gray-100 shadow-sm">
                        <div className="flex items-center gap-2">
                          <div className="flex gap-1">
                            <div className="w-2 h-2 bg-[#000000]/60 rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                            <div className="w-2 h-2 bg-[#000000]/60 rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                            <div className="w-2 h-2 bg-[#000000]/60 rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
                          </div>
                  ...
                  >
                    {FOLLOW_UP_SUGGESTIONS.map((suggestion) => (
                      <button
                        ...
                        className="px-3.5 py-2 text-xs bg-white border border-gray-200 rounded-xl text-gray-600 hover:text-[#000000] hover:border-[#000000]/30 hover:bg-[#F0F0F0]/20 transition-all duration-200 shadow-sm hover:shadow"
                      >
                        {suggestion}
                      </button>
                    ))}
        ...
        <div className="p-4 pb-5">
          <div className="max-w-3xl mx-auto">
            <form onSubmit={handleSubmit} className="relative">
              <div className="flex items-end gap-2 bg-white border border-gray-200 rounded-2xl px-4 py-3 shadow-lg shadow-gray-900/5 focus-within:ring-2 focus-within:ring-[#000000]/20 focus-within:border-[#000000]/30 transition-all duration-200">
                <textarea
                  ref={inputRef}
                  value={inputValue}
                ...
                <button
                  ...
                  className="p-2.5 bg-[#000000] hover:bg-[#1A1A1A] text-white rounded-xl transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed flex-shrink-0 shadow-sm hover:shadow-md active:scale-95"
                >
