...
import {
    xtandiIraTab as strategyInvestmentTab,
    strategicBrandsTab as realEstatePortfolioTab,
    globalMarketsTab,
    smtDigitalTab as digitalPlatformTab,
} from '@/config/clients/astellas/scenario-tabs';
import type { ScenarioTabConfig } from '@/config/clients/astellas/scenario-tabs';
...
const TAB_PLACEHOLDERS: Record<AnalysisTab, string> = {
    'pl-impact': 'e.g., What if XTANDI IRA price cut is 5pp and yen moves to ¥141/USD in H2 FY26?',
    'strategy-investment': 'e.g., What if SMT savings reach ¥45B in FY26 and PADCEV Phase 3 neoadjuvant approval is confirmed?',
    'real-estate-portfolio': 'e.g., What if VYLOY CDx penetration accelerates to 55% and China ramp exceeds ¥120B by FY26?',
    'global-markets': 'e.g., What if Japan NHI revision is limited to –2% and Established Markets PADCEV EU uptake grows 40%?',
    'digital-platform': 'e.g., What if Finance360 scenario engine adoption reaches 80% of FP&A team and R&D portfolio analytics reduce study costs by ¥3B?',
    'external-signals':
    ...
    const fetchScenarioSummary = async (
        ...
        try {
            ...
            if (tabId === 'pl-impact' && impact) {
                calculatedImpact = {
                    'Revenue Impact': `¥${impact.revenue >= 0 ? '+' : ''}${(impact.revenue / 1000).toFixed(1)}B`,
                    'Core Operating Income': `¥${impact.operatingIncome >= 0 ? '+' : ''}${(impact.operatingIncome / 1000).toFixed(1)}B`,
                    'Core Net Income': `¥${impact.netIncome >= 0 ? '+' : ''}${(impact.netIncome / 1000).toFixed(1)}B`,
                    'Core Operating Margin': `${impact.operatingMargin.toFixed(1)}%`,
                };
            }
            ...
            if (activeTab === 'pl-impact') {
                ...
                const assistantMessage: ChatMessage = {
                    ...
                    content: `${explanation}\n\nI've adjusted the following levers:\n${Object.entries(levers).map(([id, value]) => {
                        ...
                        const sign = value >= 0 ? '+' : '';
                        const formatted = unit === '¥B' ? `¥${value.toFixed(1)}B`
                            : unit === '$M' ? `$${value.toFixed(0)}M`
        ...
        <div className="bg-gray-50">
                ...
                <div className="max-w-7xl mx-auto px-6 py-6">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                            <div className="p-3 bg-[#000000] rounded-xl shadow-lg">
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <button className="px-4 py-2 bg-[#000000] text-white font-semibold rounded-lg hover:bg-[#000000] transition-all flex items-center space-x-2">
                                <Save className="w-4 h-4" />
                                <span>Save</span>
                            </button>
                            <button className="px-6 py-2 bg-[#000000] text-white font-semibold rounded-lg hover:bg-[#000000] transition-all flex items-center space-x-2">
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="flex-1 flex items-center space-x-2">
                                ...
                                <button
                                    ...
                                    className="px-4 py-2 bg-[#000000] text-white font-semibold rounded-lg hover:bg-[#000000] transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
                                >
                                    {isProcessingChat ? (
                ...
                <div className="bg-white border-b border-gray-200">
                    <div className="max-w-7xl mx-auto px-6 py-4">
                        ...
                        <div className="space-y-4 text-sm text-gray-600">
                            <p>
                                This tab mirrors the Astellas CFO driver cascade: XTANDI &amp; PADCEV franchise economics,
                                pipeline R&amp;D investment &amp; POC milestones, SMT savings trajectory &amp; SG&amp;A efficiency, and
                                FX sensitivity &amp; geographic mix — aligned to the scenario lever taxonomy on{' '}
            ...
            <AnimatePresence>
                {(isLoadingSummary || scenarioSummary) && (
                    ...
                    >
                        <div className="bg-gradient-to-br from-[#000000] to-[#000000] rounded-xl p-6 shadow-lg text-white">
                            {isLoadingSummary ? (
                    ...
                    <div className="col-span-5">
                        <div className="bg-white rounded-xl p-6 shadow-sm">
                            ...
                            {activeTab === 'external-signals' ? (
                                <div className="space-y-4 text-sm text-gray-600">
                                    <p>
                                        These five monitors distill IRA pricing risk, FX & macro exposure, competitive
                                        oncology dynamics, Japan NHI revision risk, and pipeline execution — themes
                                        documented in the Astellas research workbook, peer pharma filings, and industry outlook.
                                    </p>
                                    <ul className="list-disc pl-5 space-y-2">
                                        <li>
                                        ...
                                        <li>
                                            ...
                                            <span className="font-medium text-gray-800">P&amp;L Impact</span> (XTANDI IRA,
                                            FX sensitivity, SMT savings) or domain tabs (Strategic Investment, Global Markets, Digital Platform).
                                        </li>
                                    </ul>
                                    <p className="text-xs text-gray-500 pt-2 border-t border-gray-100">
                                        Competitive Intelligence console covers peer benchmarks; this tab focuses on
                                        uncontrollable externals that sit above Astellas&apos;s operating plan execution.
                                    </p>
                                </div>
                            ) : (
                                            ...
                                            <input
                                                ...
                                                style={{
                                                    background: `linear-gradient(to right, #e5e7eb ${((value - lever.minValue) / (lever.maxValue - lever.minValue)) * 100
                                                        }%, #000000 ${((value - lever.minValue) / (lever.maxValue - lever.minValue)) * 100
                                                        }%)`
                                                }}
                                            />
                                        </div>
                                ...
                                >
                                    {!impact && !impactError ? (
                                        <div className="bg-white rounded-xl p-6 shadow-sm flex flex-col items-center justify-center py-12">
                                            <Loader className="w-8 h-8 text-[#000000] animate-spin mb-4" />
                                    ...
                                    <div className="bg-white rounded-xl p-6 shadow-sm">
                                        <h3 className="text-lg font-semibold text-gray-900 mb-4">Astellas P&L Flow-Through Impact</h3>
                                        {isCalculating && (
                                        ...
                                        <div className="grid grid-cols-4 gap-4 mb-6">
                                            <div className="bg-gray-50 rounded-lg p-4">
                                                ...
                                                <p className={`text-xl font-bold ${impact.revenue >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                    {impact.revenue >= 0 ? '+' : ''}¥{(Math.abs(impact.revenue) / 1000).toFixed(1)}B
                                                </p>
                                            </div>
                                            <div className="bg-gray-50 rounded-lg p-4">
                                                <p className="text-xs text-gray-600 mb-1">Core Operating Income</p>
                                                <p className={`text-xl font-bold ${impact.operatingIncome >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                    {impact.operatingIncome >= 0 ? '+' : ''}¥{(Math.abs(impact.operatingIncome) / 1000).toFixed(1)}B
                                                </p>
                                            </div>
                                            <div className="bg-gray-50 rounded-lg p-4">
                                                <p className="text-xs text-gray-600 mb-1">Core Net Income Impact</p>
                                                <p className={`text-xl font-bold ${impact.netIncome >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                    {impact.netIncome >= 0 ? '+' : ''}¥{(Math.abs(impact.netIncome) / 1000).toFixed(1)}B
                                                </p>
                                            </div>
                                            <div className="bg-gray-50 rounded-lg p-4">
                                                <p className="text-xs text-gray-600 mb-1">Core Operating Margin</p>
                                        ...
                                        <div className="border-t border-gray-200 pt-4">
                                            <h4 className="text-sm font-semibold text-gray-900 mb-3">Astellas P&L Line Item Details</h4>
                                            <div className="space-y-2">
                                                {/* Revenue */}
                                                <div className="flex items-center justify-between py-2 border-b border-gray-100">
                                                    <div className="flex-1">
                                                        <p className="text-sm font-medium text-gray-900">Net Revenue</p>
                                                        <p className="text-xs text-gray-500">Base: ¥{(impact.baseRevenue / 1000).toFixed(1)}B</p>
                                                    </div>
                                                    <div className="text-right w-32">
                                                        ...
                                                        <p className="text-xs text-gray-500">
                                                            ¥{((impact.baseRevenue + impact.revenue) / 1000).toFixed(1)}B
                                                        </p>
                                                    </div>
                                                </div>
                                                {/* Revenue Segments */}
                                                <div className="ml-4 space-y-1 border-l-2 border-gray-200 pl-3">
                                                    <div className="flex items-center justify-between py-1">
                                                        <div className="flex-1">
                                                            <p className="text-xs text-gray-700">US Segment Net Sales</p>
                                                            <p className="text-xs text-gray-500">Base: ¥{(impact.baseAdvisoryServices / 1000).toFixed(0)}B</p>
                                                        </div>
                                                        <div className="text-right w-28">
                                                            <p className={`text-xs font-medium ${impact.advisoryServices >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                                {impact.advisoryServices >= 0 ? '+' : ''}¥{(Math.abs(impact.advisoryServices) / 1000).toFixed(1)}B
                                                            </p>
                                                        </div>
                                                    </div>
                                                    <div className="flex items-center justify-between py-1">
                                                        <div className="flex-1">
                                                            <p className="text-xs text-gray-700">Established Markets Net Sales</p>
                                                            <p className="text-xs text-gray-500">Base: ¥{(impact.baseBuildingOperations / 1000).toFixed(0)}B</p>
                                                        </div>
                                                        <div className="text-right w-28">
                                                            <p className={`text-xs font-medium ${impact.buildingOperations >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                                {impact.buildingOperations >= 0 ? '+' : ''}¥{(Math.abs(impact.buildingOperations) / 1000).toFixed(1)}B
                                                            </p>
                                                        </div>
                                                    </div>
                                                    <div className="flex items-center justify-between py-1">
                                                        <div className="flex-1">
                                                            <p className="text-xs text-gray-700">Japan &amp; International Net Sales</p>
                                                            <p className="text-xs text-gray-500">Base: ¥{(impact.baseProjectManagement / 1000).toFixed(0)}B</p>
                                                        </div>
                                                        <div className="text-right w-28">
                                                            <p className={`text-xs font-medium ${impact.projectManagement >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                                {impact.projectManagement >= 0 ? '+' : ''}¥{(Math.abs(impact.projectManagement) / 1000).toFixed(1)}B
                                                            </p>
                                                        </div>
                                                    </div>
                                                    <div className="flex items-center justify-between py-1">
                                                        <div className="flex-1">
                                                            ...
                                                            <p className="text-xs text-gray-500">Base: ¥{(impact.baseRealEstateInvestments / 1000).toFixed(1)}B</p>
                                                        </div>
                                                        <div className="text-right w-28">
                                                            <p className={`text-xs font-medium ${impact.realEstateInvestments >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                                {impact.realEstateInvestments >= 0 ? '+' : ''}¥{(Math.abs(impact.realEstateInvestments) / 1000).toFixed(1)}B
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center justify-between py-2 border-b border-gray-100">
                                                    <div className="flex-1">
                                                        ...
                                                        <p className="text-xs text-gray-500">Base: ¥{(impact.baseCOGS / 1000).toFixed(1)}B</p>
                                                    </div>
                                                    <div className="text-right w-32">
                                                        <p className={`text-sm font-semibold ${impact.cogs <= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                            {impact.cogs >= 0 ? '+' : ''}¥{(Math.abs(impact.cogs) / 1000).toFixed(1)}B
                                                        </p>
                                                        <p className="text-xs text-gray-500">
                                                            ¥{((impact.baseCOGS + impact.cogs) / 1000).toFixed(1)}B
                                                        </p>
                                                    </div>
                                                </div>
                                                {/* COGS Breakdown */}
                                                <div className="ml-4 space-y-1 border-l-2 border-gray-200 pl-3">
                                                    <div className="flex items-center justify-between py-1">
                                                        <p className="text-xs text-gray-700">Manufacturing Costs</p>
                                                        <p className={`text-xs font-medium ${impact.personnelCosts <= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                            {impact.personnelCosts >= 0 ? '+' : ''}¥{(Math.abs(impact.personnelCosts) / 1000).toFixed(1)}B
                                                        </p>
                                                    </div>
                                                    <div className="flex items-center justify-between py-1">
                                                        <p className="text-xs text-gray-700">Royalties & Collaboration</p>
                                                        <p className={`text-xs font-medium ${impact.subcontractorCosts <= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                            {impact.subcontractorCosts >= 0 ? '+' : ''}¥{(Math.abs(impact.subcontractorCosts) / 1000).toFixed(1)}B
                                                        </p>
                                                    </div>
                                                    <div className="flex items-center justify-between py-1">
                                                        <p className="text-xs text-gray-700">Other COGS</p>
                                                        <p className={`text-xs font-medium ${impact.facilityCosts <= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                            {impact.facilityCosts >= 0 ? '+' : ''}¥{(Math.abs(impact.facilityCosts) / 1000).toFixed(1)}B
                                                        </p>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center justify-between py-2 border-b border-gray-100 bg-gray-50 rounded px-3 -mx-3">
                                                    ...
                                                    <div className="text-right w-32">
                                                        <p className={`text-sm font-bold ${impact.grossProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                            {impact.grossProfit >= 0 ? '+' : ''}¥{(Math.abs(impact.grossProfit) / 1000).toFixed(1)}B
                                                        </p>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center justify-between py-2 border-b border-gray-100">
                                                    <div className="flex-1">
                                                        ...
                                                        <p className="text-xs text-gray-500">Base: ¥{(impact.baseOpEx / 1000).toFixed(1)}B</p>
                                                    </div>
                                                    <div className="text-right w-32">
                                                        <p className={`text-sm font-semibold ${impact.opEx <= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                            {impact.opEx >= 0 ? '+' : ''}¥{(Math.abs(impact.opEx) / 1000).toFixed(1)}B
                                                        </p>
                                                        <p className="text-xs text-gray-500">
                                                            ¥{((impact.baseOpEx + impact.opEx) / 1000).toFixed(1)}B
                                                        </p>
                                                    </div>
                                                </div>
                                                {/* OpEx Breakdown */}
                                                <div className="ml-4 space-y-1 border-l-2 border-gray-200 pl-3">
                                                    <div className="flex items-center justify-between py-1">
                                                        <div className="flex-1">
                                                            <p className="text-xs text-gray-700">R&D Expense</p>
                                                            <p className="text-xs text-gray-500">Base: ¥{(impact.baseTechnologyCosts / 1000).toFixed(1)}B</p>
                                                        </div>
                                                        <p className={`text-xs font-medium ${impact.technologyCosts <= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                            {impact.technologyCosts >= 0 ? '+' : ''}¥{(Math.abs(impact.technologyCosts) / 1000).toFixed(1)}B
                                                        </p>
                                                    </div>
                                                    <div className="flex items-center justify-between py-1">
                                                        <div className="flex-1">
                                                            <p className="text-xs text-gray-700">Selling Expenses</p>
                                                            <p className="text-xs text-gray-500">Base: ¥{(impact.baseMarketing / 1000).toFixed(1)}B</p>
                                                        </div>
                                                        <p className={`text-xs font-medium ${impact.marketing <= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                            {impact.marketing >= 0 ? '+' : ''}¥{(Math.abs(impact.marketing) / 1000).toFixed(1)}B
                                                        </p>
                                                    </div>
                                                    <div className="flex items-center justify-between py-1">
                                                        <div className="flex-1">
                                                            <p className="text-xs text-gray-700">Pipeline Investment</p>
                                                            <p className="text-xs text-gray-500">Base: ¥{(impact.baseProfessionalDev / 1000).toFixed(1)}B</p>
                                                        </div>
                                                        <p className={`text-xs font-medium ${impact.professionalDev <= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                            {impact.professionalDev >= 0 ? '+' : ''}¥{(Math.abs(impact.professionalDev) / 1000).toFixed(1)}B
                                                        </p>
                                                    </div>
                                                    <div className="flex items-center justify-between py-1">
                                                        <div className="flex-1">
                                                            <p className="text-xs text-gray-700">SG&A</p>
                                                            <p className="text-xs text-gray-500">Base: ¥{(impact.baseSGandA / 1000).toFixed(1)}B</p>
                                                        </div>
                                                        <p className={`text-xs font-medium ${impact.sga <= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                            {impact.sga >= 0 ? '+' : ''}¥{(Math.abs(impact.sga) / 1000).toFixed(1)}B
                                                        </p>
                                                    </div>
                                                    <div className="flex items-center justify-between py-1">
                                                        <div className="flex-1">
                                                            <p className="text-xs text-gray-700">Other Operating Expenses</p>
                                                            <p className="text-xs text-gray-500">Base: ¥{(impact.baseOtherOpEx / 1000).toFixed(1)}B</p>
                                                        </div>
                                                        <p className={`text-xs font-medium ${impact.otherOpEx <= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                            {impact.otherOpEx >= 0 ? '+' : ''}¥{(Math.abs(impact.otherOpEx) / 1000).toFixed(1)}B
                                                        </p>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center justify-between py-2 border-b border-gray-100 bg-[#F0F0F0] rounded px-3 -mx-3">
                                                    <div>
                                                        <p className="text-sm font-bold text-[#000000]">Operating Income</p>
                                                    ...
                                                    <div className="text-right w-32">
                                                        ...
                                                        <p className="text-xs text-gray-600 font-semibold">
                                                            ¥{((impact.baseOperatingIncome + impact.operatingIncome) / 1000).toFixed(1)}B
                                                        </p>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center justify-between py-2 border-b border-gray-100">
                                                    <div className="flex-1">
                                                        ...
                                                        <p className="text-xs text-gray-500">Base: ¥{(impact.baseInterest / 1000).toFixed(1)}B</p>
                                                    </div>
                                                    <p className={`text-xs font-medium ${impact.interest <= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                        {impact.interest >= 0 ? '+' : ''}¥{(Math.abs(impact.interest) / 1000).toFixed(1)}B
                                                    </p>
                                                </div>
                                                <div className="flex items-center justify-between py-2 border-b border-gray-100">
                                                    <div className="flex-1">
                                                        ...
                                                        <p className="text-xs text-gray-500">Base: ¥{(impact.baseTax / 1000).toFixed(1)}B</p>
                                                    ...
                                                    <div className="text-right">
                                                        <p className="text-xs font-medium text-slate-600 tabular-nums">
                                                            {impact.tax >= 0 ? '+' : ''}¥{(Math.abs(impact.tax) / 1000).toFixed(1)}B
                                                        </p>
                                                    </div>
                                                </div>

                                                {/* Net Income */}
                                                <div className="flex items-center justify-between py-2 bg-[#000000] rounded px-3 -mx-3">
                                                    <div>
                                                        <p className="text-sm font-bold text-white">Core Net Income</p>
                                                        <p className="text-xs text-gray-300">Core EPS Impact: {impact.netIncome >= 0 ? '+' : ''}¥{(impact.netIncome / 4400).toFixed(2)}</p>
                                                    </div>
                                                    <div className="text-right w-32">
                                                        ...
                                                        <p className="text-xs text-gray-300">
                                                            ¥{((impact.baseNetIncome + impact.netIncome) / 1000).toFixed(1)}B
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
