        ...
        return (
            <button
                ...
                className={`w-full text-left pl-2 py-1.5 rounded-md text-sm flex items-center gap-2 ${
                    active ? 'bg-[#000000]/10 text-[#000000] font-medium' : 'text-gray-700 hover:bg-gray-50'
                }`}
...
export default function DriverAnalyticsTab({ levers }: DriverAnalyticsTabProps) {
    const [expanded, setExpanded] = useState<Set<string>>(
        () => new Set(['cfo-root', 'xtandi-franchise', 'xtandi-ira-mgmt']),
    );
    const [selectedId, setSelectedId] = useState<string | null>('xtandi-ira-maximum-fair-price');
        ...
        <div className="space-y-4">
            <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
                <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                    <GitBranch className="w-5 h-5 text-[#000000]" />
                    Driver analytics — CFO cascade
                </h2>
                <p className="text-sm text-gray-600 mt-1">
                    Tree follows the Astellas CFO driver cascade behind scenario levers. Each leaf includes an illustrative
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="lg:col-span-2 bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-4 min-w-0">
                    {!selectedId || !node ? (
                        ...
                        <>
                            <div>
                                <h3 className="text-base font-bold text-[#000000]">{meta?.name ?? node.label}</h3>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                ...
                                <div className="rounded-lg bg-gray-50 border border-gray-100 p-3">
                                    ...
                                    <p className="text-2xl font-bold text-[#000000] mt-0.5">{score}</p>
