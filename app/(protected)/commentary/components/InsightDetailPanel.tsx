            ...
            <div className="sticky top-0 bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between z-10">
              <div className="flex items-center gap-2">
                {item.source === 'ai' ? (
                  ...
                  <PillBadge className="bg-[#F0F0F0] text-[#000000]">
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                ...
                {item.kpiValue && (
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-xl font-bold text-[#000000]">{item.kpiValue}</span>
                  </div>
                )}
              </div>
                ...
                <div className="flex items-center gap-1 flex-wrap text-xs text-gray-500">
                  {item.driverPath.map((p, i) => (
                    <span key={i} className="flex items-center gap-1">
                      ...
                      <span className={i === item.driverPath.length - 1 ? 'text-[#000000] font-medium' : ''}>
                        {p}
                      </span>
                    </span>
                  ))}
              ...
              {item.source === 'human' && item.authorName && (
                <div className="flex items-center gap-3 bg-gray-50 rounded-lg p-3">
                  <div className="w-8 h-8 rounded-full bg-[#000000] text-white flex items-center justify-center text-xs font-semibold">
                    {item.authorName.split(' ').map(n => n[0]).join('').toUpperCase()}
                  </div>
                  <div>
                    <p className="text-sm font-medium text-[#000000]">{item.authorName}</p>
                    ...
                    {((item.rawInsight.relatedDrivers as Record<string, unknown>).impactedMetrics as Array<{metric: string; value: string; trend: string}>).map((m, i) => (
                      <div key={i} className="flex items-center justify-between text-xs bg-gray-50 rounded px-2 py-1.5">
                        ...
                        <div className="flex items-center gap-1">
                          <span className="font-medium text-[#000000]">{m.value}</span>
                  ...
                  <ol className="space-y-2">
                    {item.recommendations.map((r, i) => (
                      <li key={i} className="flex items-start gap-2 text-sm text-gray-700">
                        <span className="w-5 h-5 rounded-full bg-[#F0F0F0] text-[#000000] flex items-center justify-center shrink-0 text-xs font-semibold mt-0.5">
                          {i + 1}
                        </span>
                        {r}
                      </li>
              ...
              {item.consoleLink && (
                <a
                  ...
                  className="inline-flex items-center gap-1.5 text-sm text-[#000000] hover:text-[#1A1A1A] font-medium"
                >
                    ...
                    {relatedItems.slice(0, 8).map(ri => (
                      ...
                      >
                        {ri.source === 'ai' ? (
                          ...
                          <User className="w-3.5 h-3.5 text-[#000000] shrink-0 mt-0.5" />
                        )}
                        <div className="min-w-0 flex-1">
                          <p className="text-xs font-medium text-[#000000] truncate">{ri.title}</p>
