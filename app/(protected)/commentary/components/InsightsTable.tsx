...
export default function InsightsTable({ items, selectedId, onSelect }: InsightsTableProps) {
  ...
  const SortIcon = ({ field }: { field: SortField }) => {
    ...
    return sortDir === 'asc'
      ? <ArrowUp className="w-3 h-3 text-[#000000]" />
      : <ArrowDown className="w-3 h-3 text-[#000000]" />;
  };
            ...
            return (
              <div
                ...
                className={`absolute left-0 right-0 flex items-center text-xs border-b border-gray-100 cursor-pointer transition-colors ${
                  isSelected
                    ? 'bg-[#F0F0F0]/60 border-l-2 border-l-[#000000]'
              ...
              >
                ...
                <div className="flex-1 min-w-[200px] pr-2 overflow-hidden">
                  <p className="truncate text-[#000000] font-medium">{item.title}</p>
                </div>
                ...
                {/* KPI value */}
                <div className="w-16 shrink-0 font-semibold text-[#000000] tabular-nums">
                  {item.kpiValue}
                </div>
