 ...
 * Overlays Astellas Pharma seed financials from config (Annual Report / IR–aligned) only where slices are empty or zeroed.
 *
 * Segments are always taken from the seed config. The DB may contain legacy
 * placeholder segments; using the seed config guarantees the correct Astellas Pharma
 * segment breakdown (United States, Established Markets, Japan, International Markets, China) is always shown.
 */

import type { FinancialConfig } from '@/config/types';
import { financials as seed } from '@/config/clients/astellas/financials';
...
export function ensureExecutiveOverviewFinancials(fin: FinancialConfig): FinancialConfig {
    ...
    // (Q1 FY25 – Q1 FY26) and are always fully populated with Astellas Pharma actuals.
