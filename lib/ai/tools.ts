...
export const aiTools = {
  ...
  searchDatabase: tool({
    description: 'Search across all Astellas Pharma Inc. financial data including KPIs, financials, insights, commentary, segments, initiatives, variance explanations, and more. Use this for any data lookup. (L1: Facts & Trends)',
    inputSchema: z.object({
      query: z.string().describe('Search query (e.g., "XTANDI revenue", "Core OP margin", "PADCEV volume", "Core EPS", "Japan NHI", "SMT savings", "IRA MFP", "FX USD/JPY")'),
    }),
    execute: async ({ query }) => {
  ...
  getKPIDashboard: tool({
    description: 'Get all current KPIs organized by category (primary, operational, digital, financial). Includes Astellas Pharma operating metrics (XTANDI revenue, PADCEV revenue, Core OP margin, Core EPS, SMT savings), segment metrics (US, Japan, Established Markets, International Markets), and financial KPIs. Use this for overview questions. (L1: Facts & Trends)',
    inputSchema: z.object({}),
    execute: async () => {
      const companyId = await getActiveCompanyId();
  ...
  getFinancialPerformance: tool({
    description: 'Get detailed financial data including quarterly results, segment breakdown (US: XTANDI + PADCEV + VEOZAH + IZERVAY; Japan: XTANDI Japan + VYLOY; Established Markets: Europe/Australia/Canada; International Markets), P&L, revenue bridge, financial ratios, and working capital. (L1: Facts & Trends)',
    inputSchema: z.object({}),
    execute: async () => {
      const companyId = await getActiveCompanyId();
  ...
  getStrategicOverview: tool({
    description: 'Get strategic initiatives (XTANDI IRA MFP mitigation, PADCEV manufacturing expansion, VEOZAH launch acceleration, VYLOY Japan launch, SMT ¥40B savings delivery), risks, forward outlook, and key opportunities for Astellas Pharma Inc.',
    inputSchema: z.object({}),
    execute: async () => {
      const companyId = await getActiveCompanyId();
  ...
  getActiveAlerts: tool({
    description: 'Get all configured alert templates with thresholds, severity levels, and suggested actions for regulatory indicators and Astellas Pharma Inc. operational metrics.',
    inputSchema: z.object({}),
    execute: async () => {
      const companyId = await getActiveCompanyId();
  ...
  getForecastData: tool({
    description: 'Get ML forecast results with confidence intervals and accuracy metrics for Astellas Pharma Inc. financial and operational metrics. Use for L3 (Forecast Achievement) questions. (L3: Forecast Achievement)',
    inputSchema: z.object({
      metricName: z.string().optional().describe('Filter by metric name (e.g., "XTANDI Revenue", "Core OP", "PADCEV Volume", "Core EPS", "SMT Savings")'),
  ...
  getAnomalyDetections: tool({
    description: 'Get detected anomalies in financial and operational metrics with severity, explanation, and related drivers. Monitors XTANDI US revenue deviations, PADCEV volume changes, Core OP margin vs. plan, Japan NHI revision timing gaps, and SMT/SG&A exceptions. (L1/L5: Facts & Root Cause)',
    inputSchema: z.object({
  ...
  analyzeVariance: tool({
    description: 'Perform driver-based variance analysis for Astellas Pharma Inc. metrics. Includes pre-computed variance explanations with quantified driver impacts, financial trends, in-cycle estimates, and anomalies. Covers XTANDI revenue, PADCEV volume, Core OP margin, Japan NHI revision impact, IRA MFP, and more. (L2/L5: Variance & Root Cause)',
    inputSchema: z.object({
      metricName: z.string().describe('The metric to analyze (e.g., "XTANDI US revenue", "Core OP", "PADCEV volume", "Japan segment revenue", "SMT savings", "Core EPS", "FX impact")'),
    }),
  ...
  forecastAchievement: tool({
    description: 'Compare current-period actuals against forecast, budget, and prior year. Shows in-cycle flash estimates and ML forecasts for Astellas metrics including XTANDI revenue, Core EPS, PADCEV volume, Japan NHI impact, SMT savings, and key operating metrics. (L3: Forecast Achievement)',
    inputSchema: z.object({
      metricName: z.string().optional().describe('Optional metric filter (e.g., "XTANDI Revenue", "Core EPS", "Core OP", "PADCEV Volume", "SMT Savings")'),
    }),
    execute: async ({ metricName }) => {
      const companyId = await getActiveCompanyId();
  ...
  analyzeRegionalPerformance: tool({
    description: 'Get geographic segment performance data across Astellas Pharma operating segments (United States, Japan, Established Markets, International Markets, China). Shows revenue, growth, operating margin, and product mix by segment. Returns latest quarter only for conciseness.',
    inputSchema: z.object({
      region: z.string().optional().describe('Optional segment filter (e.g., "United States", "Japan", "Established Markets", "International Markets", "China")'),
    }),
  ...
  analyzeProductMix: tool({
    description: 'Get revenue product performance data across Astellas product categories (XTANDI Franchise, PADCEV, VEOZAH, IZERVAY, VYLOY, Other Oncology). Shows revenue, mix %, growth, and operating metrics by product franchise. Use for revenue mix and product performance questions.',
    inputSchema: z.object({
      category: z.string().optional().describe('Optional product filter (e.g., "XTANDI", "PADCEV", "VEOZAH", "IZERVAY", "VYLOY", "Other Oncology")'),
    }),
  ...
  analyzeDaypartPerformance: tool({
    description: 'Get seasonality and fiscal period performance data for Astellas Pharma Inc. Analyzes performance across pharmaceutical cycles — Q1 Japan NHI revision effective date, Q2/Q3 XTANDI/PADCEV peak enrollment, Q4 earnings guidance and pipeline readouts. Shows revenue, volume, and margin by fiscal period.',
    inputSchema: z.object({
      daypart: z.string().optional().describe('Optional period filter (e.g., "Japan NHI Revision Quarter", "PADCEV Peak Enrollment", "XTANDI IRA Transition", "SMT Savings Acceleration")'),
    }),
  ...
  analyzeCostDrivers: tool({
    description: 'Get cost driver decomposition showing COGS, R&D, SG&A, D&A, and Interest for Astellas Pharma Inc. Shows amount, % of revenue, YoY change, and variance to budget. Returns latest quarter only.',
    inputSchema: z.object({
      costCategory: z.string().optional().describe('Optional category filter (e.g., "Cost of Goods Sold", "R&D Expense", "SG&A", "SMT Savings", "D&A", "Interest Expense")'),
    }),
  ...
  getWeeklyTrends: tool({
    description: 'Get weekly KPI snapshots for the current quarter (Q2 FY26) showing week-over-week and year-over-year trends for XTANDI US prescriptions, PADCEV infusion volume, VEOZAH weekly TRx growth, IZERVAY specialist scripts, and SMT savings tracking.',
    inputSchema: z.object({
      metricName: z.string().optional().describe('Optional metric filter (e.g., "XTANDI Weekly TRx", "PADCEV Infusion Volume", "VEOZAH Weekly TRx", "IZERVAY Specialist Scripts", "SMT Savings Run-Rate")'),
    }),
    execute: async ({ metricName }) => {
      const companyId = await getActiveCompanyId();
  ...
  getStorePortfolioAnalysis: tool({
    description: 'Get channel and payer analysis showing performance by Astellas customer segment (Medicare Part D, Commercial Insurance, Japan NHI, EU Reimbursement, Self-Pay/OTC). Shows avg revenue per patient, volume, net price, satisfaction, and margin by channel cohort.',
    inputSchema: z.object({}),
    execute: async () => {
      const companyId = await getActiveCompanyId();
  ...
  getSensitivityAnalysis: tool({
    description: 'Get elasticity/sensitivity factors showing how changes in one metric impact another for Astellas Pharma Inc. For example: "+1% XTANDI US volume ≈ +¥2 Core EPS" or "+¥1 USD/JPY ≈ +¥2.1B Core OP". Use for what-if and sensitivity questions.',
    inputSchema: z.object({
      driverMetric: z.string().optional().describe('Optional driver metric filter (e.g., "XTANDI US Volume Growth", "IRA MFP Discount", "Japan NHI Revision Rate", "USD/JPY Rate", "PADCEV Manufacturing Capacity")'),
    }),
  ...
  getCompetitiveBenchmarking: tool({
    description: 'Get competitive benchmarking data for Astellas\'s global pharma peers (AstraZeneca, Johnson & Johnson Pharma, Novartis, Roche, Pfizer). Shows revenue, operating margin, pipeline advancement, and EPS CAGR by competitor by quarter where populated.',
    inputSchema: z.object({
      competitorName: z.string().optional().describe('Optional competitor filter (e.g., "AstraZeneca", "Johnson & Johnson", "Novartis", "Roche", "Pfizer")'),
    }),
  ...
  exploreDriverGraph: tool({
    description: 'Traverse the driver relationship graph to find upstream causes, downstream impacts, or cross-console relationships for a given driver. Returns computed metrics with numeric values, health scores, causal weights, and impact paths. Use for root-cause analysis of Astellas Pharma Inc. commercial and financial performance. (L5: Root Cause)',
    inputSchema: z.object({
      driverId: z.string().describe('The driver ID to explore (e.g., "xtandi-us-net-revenue-growth", "padcev-manufacturing-capacity", "japan-nhi-price-revision-pct", "fx-usd-jpy-rate", "smt-savings-target-m")'),
  ...
  getFXImpactAnalysis: tool({
    description: 'Get foreign exchange impact analysis showing currency pair headwinds/tailwinds on revenue and Core OP. Astellas Pharma Inc. is a Japan-headquartered company with ~60% of revenue in USD/EUR — each ¥1 USD/JPY move impacts Core OP by approximately ¥2.1B annually. Returns detailed FX sensitivity by currency pair and segment.',
    inputSchema: z.object({}),
    execute: async () => {
      const companyId = await getActiveCompanyId();
      ...
      return {
        totalRevenueImpact: `¥${totalRevenueImpact}B`,
        totalOperatingImpact: `¥${totalOperatingImpact}B`,
        byPair: data.map((fx: any) => ({
          currencyPair: fx.currencyPair, segment: fx.segment,
          revenueImpact: `¥${fx.revenueImpact}B`,
          operatingImpact: `¥${fx.operatingImpact}B`,
  ...
  getCompetitorDeepDive: tool({
    description: 'Get enhanced competitive analysis combining quarterly peer metrics (revenue, margin, pipeline, and other tracked items) with market overview data (global oncology market size, share, drivers, challenges). Use for competitive positioning and share questions vs. AstraZeneca, Johnson & Johnson, Novartis, Roche, and Pfizer.',
    inputSchema: z.object({
      competitorName: z.string().optional().describe('Optional peer filter (e.g., "AstraZeneca", "Johnson & Johnson", "Novartis", "Roche", "Pfizer")'),
    }),
    execute: async ({ competitorName }) => {
      const companyId = await getActiveCompanyId();
