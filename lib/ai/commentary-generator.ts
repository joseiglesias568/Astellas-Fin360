...
const PERSONAS: Persona[] = [
    ...
    },
    {
        ...
        focusAreas: ['FX hedging', 'IRA price risk', 'compliance', 'liquidity management'],
    },
    {
        name: 'Atsushi Kitamura',
        role: 'CFO, Astellas Pharma Inc.',
        categories: ['Financial Performance', 'Revenue & Market', 'Strategic'],
        voiceTraits: ['strategic', 'concise', 'board-level'],
        focusAreas: ['Core OP impact', 'capital allocation', 'shareholder value', 'long-term targets'],
    },
    {
        name: 'Lisa Wang',
        role: 'VP, Global Regulatory Affairs',
        categories: ['Revenue & Market'],
        voiceTraits: ['regulatory-savvy', 'policy-oriented', 'compliance-focused'],
        focusAreas: ['IRA price negotiation strategy', 'PMDA regulatory strategy', 'Japan NHI pricing', 'FDA label expansions', 'PADCEV HTA market access'],
    },
    {
        name: 'David Park',
        role: 'VP, US Oncology Commercial',
        categories: ['Operations & Service Delivery'],
        voiceTraits: ['operational', 'hands-on', 'efficiency-focused'],
        focusAreas: ['XTANDI prostate cancer share', 'PADCEV bladder cancer launch', 'VEOZAH women\'s health', 'HCP engagement', 'prescriber adoption'],
    },
    {
        name: 'Amanda Foster',
        ...
        focusAreas: ['HCP digital engagement', 'SMT digital commercial adoption', 'CRM analytics', 'omnichannel personalization'],
    },
    {
        name: 'James Wilson',
        ...
        focusAreas: ['PADCEV biologics manufacturing', 'ADC supply chain efficiency', 'procurement savings', 'vendor management'],
    },
    {
        name: 'Priya Sharma',
...
const FISCAL_PERIODS = [
    { period: 'Q4 FY25', periodType: 'quarter' as const, dateRangeStart: new Date('2026-01-05').getTime(), dateRangeEnd: new Date('2026-03-20').getTime() },
    { period: 'Q1 FY26', periodType: 'quarter' as const, dateRangeStart: new Date('2026-04-05').getTime(), dateRangeEnd: new Date('2026-06-15').getTime() },
];
...
function getPersonaOpenings(persona: Persona, rng: () => number): string[] {
    const openersByRole: Record<string, string[]> = {
        ...
        'VP, Global Regulatory Affairs': [
            ...
            'Our IRA and PMDA analysis indicates',
            'The regulatory framework suggests',
            'CMS and MHLW filings show',
            'Regulatory intelligence confirms',
        ],
        'VP, US Oncology Commercial': [
            'On the commercial operations side,',
            'Field reports confirm that',
            'Our XTANDI and PADCEV prescriber data shows',
            'Commercial metrics indicate',
...
function generateContext(
    ...
    const contextFactors = [
        `The broader oncology sector saw ${rng() > 0.5 ? 'continued pipeline momentum as prostate and bladder cancer treatment advances accelerated across global markets' : 'increased competitive intensity as ERLEADA and LYNPARZA expanded prostate cancer indications, creating headwinds for XTANDI share in specific patient segments'} during the period.`,
        `XTANDI IRA price negotiation continued ${rng() > 0.5 ? 'through the CMS engagement process with the Maximum Fair Price announcement expected mid-2025 — management has guided a ¥30-80B annual revenue headwind effective January 2026' : 'to be the primary US revenue risk, with Medicare Part D representing ~35-40% of XTANDI US revenue subject to the MFP mechanism'}.`,
        `Japan NHI revision dynamics ${rng() > 0.5 ? 'were favorable for the quarter, with pre-revision period sustaining full WAC pricing on XTANDI and PADCEV ahead of April 2026 effective date' : 'created uncertainty as MHLW pricing review signals indicated a -6.5% revision, wider than the -5.5% FY25 revision, creating planning uncertainty'}.`,
        `The global pharmaceutical market ${rng() > 0.5 ? 'remained constructive, with PADCEV first-line bladder cancer standard-of-care adoption proceeding in line with EV-302 landmark data trajectory' : 'introduced some competition as Syfovre (Apellis) made aggressive pricing moves in geographic atrophy, creating IZERVAY market share headwinds'}.`,
        `Macroeconomic conditions including ${rng() > 0.5 ? 'USD/JPY at ¥152 providing favorable XTANDI US revenue translation vs yen-reported Core OP' : 'Bank of Japan rate normalization signals creating JPY appreciation risk toward ¥140 representing a ~¥25B annual Core OP headwind'} provided the backdrop for this quarter's results.`,
        `Seasonal patterns aligned with launch trajectory norms, with ${rng() > 0.5 ? 'Q2-Q3 reflecting strong PADCEV infusion center adoption from growing US urology and medical oncology prescriber base' : 'Q1 typically reflecting modest VEOZAH script seasonality as OB/GYN prescribing builds through mid-year patient awareness campaigns'}.`,
    ];
...
function generateRecommendation(
    ...
    const recommendations = [
        ...
        `Reallocate **¥${Math.floor(rng() * 15 + 5)}B** from lower-priority discretionary spend toward ${driver.name} improvement programs`,
        `Launch a targeted pilot program in our top ${Math.floor(rng() * 50 + 50)} prescriber accounts to validate the proposed intervention before system-wide deployment`,
...
function generateTags(console: SemanticConsole, driver: SemanticDriver, metrics: SemanticMetric[], rng: () => number): string[] {
    ...
    const consoleTagMap: Record<string, string[]> = {
        'xtandi-franchise': ['xtandi', 'prostate-cancer', 'ira-mfp', 'enzalutamide'],
        'padcev-commercial': ['padcev', 'bladder-cancer', 'adc', 'enfortumab'],
        'veozah-launch': ['veozah', 'fezolinetant', 'womens-health', 'hot-flashes'],
        'izervay-commercial': ['izervay', 'geographic-atrophy', 'complement', 'retina'],
        'vyloy-japan': ['vyloy', 'zolbetuximab', 'gastric-cancer', 'cldn18'],
        'japan-segment': ['japan-nhi', 'mhlw', 'nhilisting', 'price-revision'],
        'established-markets': ['eu-launch', 'hta', 'reimbursement', 'europe'],
        'smt-transformation': ['smt', 'digital-commercial', 'sga-efficiency', 'savings'],
        'us-commercial': ['us-oncology', 'prescriber-adoption', 'market-share', 'erleada'],
        'pipeline-rd': ['pipeline', 'phase3', 'oncology', 'gene-therapy'],
        'workforce-culture': ['talent', 'retention', 'training', 'safety'],
        'fx-treasury': ['usd-jpy', 'fx-hedging', 'currency', 'treasury'],
        'sustainability-esg': ['esg', 'carbon', 'sustainability', 'tcfd'],
        'financial-performance': ['core-op', 'core-eps', 'margin', 'guidance'],
    };
